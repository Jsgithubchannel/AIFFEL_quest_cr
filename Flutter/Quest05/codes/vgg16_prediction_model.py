...
async def prediction_model(model):
    img_path = os.path.expanduser('./jellyfish_img.png')
    if not os.path.exists(img_path):
        raise FileNotFoundError(f"Image file not found: {img_path}")
    
    img = Image.open(img_path)
    # ValueError: Input 0 is incompatible with layer vgg16: expected shape=(None, 224, 224, 3), found shape=(None, 224, 224, 4) 해결
    if img.mode == 'RGBA':
        img = img.convert('RGB')
